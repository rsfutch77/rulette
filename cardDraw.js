...
function showCloneCardModal(cloneCard) {
    ...
    if (!modal) {
        ...
        window.showNotification('Clone card modal not found.', 'Error');
        return;
    }
    ...
    if (window.gameManager.cardManager && window.gameManager.cardManager.cloneCard) {
        try {
            ...
            if (result.success) {
                ...
                window.showNotification('Successfully cloned "' + targetCard.name + '" from ' + targetPlayer.displayName, 'Card Cloned!');
                ...
                window.showNotification(result.error || 'Failed to clone card', 'Clone Failed');
            }
        } catch (error) {
            console.error("[CLONE] Error during clone operation:", error);
            window.showNotification('An error occurred while cloning the card', 'Error');
        }
    } else {
        console.error("[CLONE] Card manager clone functionality not available");
        window.showNotification('Clone functionality not available', 'Error');
    }
}

/**
 * Hide the clone card modal
 */
...
// Set up event handlers for clone modal when DOM is loaded
